        ...
        try:
            ...
            if "vehicle" in steps and bags:
                ...
                for attempt in range(3):
                    cur.execute(
                        "SELECT id, create_time FROM product_plan_bill_basic WHERE bill_lading_no=%s AND country_code='MN' LIMIT 1",
                        (plate,)
                    )
                    row=cur.fetchone()
                    print(f"[VEHICLE QUERY] plate={plate} attempt={attempt+1} row={row}")
                    if row:
                        break
                    await asyncio.sleep(1)
                if not row:
                    async for m in sse("step",{"step":"装车","key":"vehicle","status":"fail","msg":"未查到车辆ID"}): yield m
                    async for m in sse("error",{"message":f"装车-未查到车辆ID (plate={plate})"}): yield m
                    return
                vid=row[0]
